...
terraform {
  backend "s3" {
    ...
    encrypt = true
    region  = "eu-west-2"
    key     = "services/academy-resident-information-api/state" #e.g. "services/transactions-api/state"
  }
}
/*    POSTGRES SET UP    */
...
module "postgres_db_development" {
  ...
  db_identifier = "academy-mirror"
  db_instance_class = "db.t2.micro"
  db_name = "academy-mirror"
  db_port  = 5002
  db_username = "${local.parameter_store}/academy-api/development/postgres-username" 
  db_password = "${local.parameter_store}/academy-api/development/postgres-password" 
